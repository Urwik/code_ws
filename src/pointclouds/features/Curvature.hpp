...
namespace fjs{
  ...
  class Curvature{
    ...
    public:
      ...
      void setCentroid(typename pcl::PointT centroid) { this->centroid_ = centroid; }
      void setInputCloud(typename pcl::PointCloud<PointT>::Ptr cloud) { this->cloud_ = cloud; }
      void setIndices(typename pcl::PointIndices::Ptr indices) { this->indices_ = indices; }
      pcl::PointIndices::Ptr getIndices() { return this->indices_; }

      void set_knn(int knn);
      void set_radius(float radius);
      void set_search_method(typename pcl::search::Search<PointT>::Ptr search);
